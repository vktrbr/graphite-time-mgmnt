...
1. Точность предсказаний. TODO: Определить метрику точности с учетом асимметрии ошибок. Нам менее
   важно переоценить время выполнения задачи, чем недооценить. (Weighted Accuracy > 50%)
...
- Метрики Критерии успеха:
    - Точность предсказаний Weighted Accuracy > 50%. (Текущая метрика с ручной оценкой ~ 33%)
